  ...
  return (
    <div className="flex flex-col h-screen max-h-screen overflow-hidden select-none" style={{ background: "hsl(220 18% 7%)" }}>
      ...
      <div className="flex flex-1 min-h-0">
        {/* Sidebar — narrow, icon above label */}
        <div className="w-[100px] shrink-0 flex flex-col items-center pt-6 gap-1" style={{ background: "hsl(220 18% 8%)" }}>
          {sidebarItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`relative w-full flex flex-col items-center gap-1.5 py-4 transition-all ${
                  active ? "text-primary" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                {active && (
                  <div className="absolute left-0 top-2 bottom-2 w-[3px] rounded-r-full bg-primary shadow-lg shadow-primary/50" />
                )}
                <Icon active={active} />
      ...
      {/* Bottom promo banner — pink/salmon gradient like original */}
      <div className="h-[52px] shrink-0 flex items-center px-4 relative overflow-hidden" style={{ background: "linear-gradient(135deg, hsl(350 60% 65%), hsl(20 70% 55%))" }}>
        {/* Mascot placeholder */}
